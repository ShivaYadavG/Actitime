...
public class CommonTest extends BaseActi {
	...
	public void LoginActiTest() throws IOException, InterruptedException {
		...
		actions.moveToElement(driver.findElement(By.xpath(TasksTabPage.createProject()))).perform(); // using move to
																										// element bcz
																										// element is
		...
		// creating new task
		actions.moveToElement(driver.findElement(By.xpath(TasksTabPage.addNewBtn()))).perform(); // using move to
																									// element bcz
																									// element is
																									// clickInterceptedException
		wait.until(ExpectedConditions.elementToBeClickable(driver.findElement(By.xpath(TasksTabPage.addNewBtn()))));
...
//		String customerNameAfterSearching = driver.findElement(By.xpath(TasksTabPage.customerName())).getText();
		// Find all elements with a specific xpath selector (change this according to
		// your requirement)
		List<WebElement> elements = driver.findElements(By.xpath(TasksTabPage.customerNameContainer()));
		for (WebElement element : elements) {
			String customerNameAfterSearching = element.getText();
			if (customerNameAfterSearching.equalsIgnoreCase(prop.getProperty("customerName"))) {
